...
def parameter_injection_attack(alice, bob):
    ...
    _msg = b'Hello i am A, how are you?'
    ...
    try:
        print(f"actual message: {_msg.decode()}")
        print(f"message_a decoded by mitm: {mitm_hacked_message_a.decode()}")
        print(f"message_b decoded by mitm: {mitm_hacked_message_b.decode()}")
        print("Attack Worked!, as MITM can able to decrypt the message")
    except:
        print("failed")
